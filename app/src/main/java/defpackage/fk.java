...
final class fk {
    ...
    public final void d(MenuItem menuItem) {
        ...
        if (this.z != null) {
            fl flVar = this.E;
            Context context = flVar.e;
            if (context.isRestricted()) {
                throw new IllegalStateException("The android:onClick attribute cannot be used within a restricted context");
            }
            if (flVar.f == null) {
                flVar.f = flVar.a(context);
            }
            menuItem.setOnMenuItemClickListener(new fj(flVar.f, this.z));
        }
        if (this.r >= 2) {
            if (menuItem instanceof gd) {
                ((gd) menuItem).j(true);
            } else if (menuItem instanceof gi) {
                gi giVar = (gi) menuItem;
                try {
                    if (giVar.d == null) {
                        giVar.d = giVar.c.getClass().getDeclaredMethod("setExclusiveCheckable", Boolean.TYPE);
                    }
                    giVar.d.invoke(giVar.c, true);
                } catch (Exception e) {
        ...
        String str = this.x;
        if (str != null) {
            menuItem.setActionView((View) b(str, fl.a, this.E.c));
            z = true;
        }
        int i2 = this.w;
        if (i2 > 0) {
        ...
        if (coiVar != null) {
            if (menuItem instanceof cmn) {
                ((cmn) menuItem).d(coiVar);
            } else {
                Log.w("MenuItemCompat", "setActionProvider: item does not implement SupportMenuItem; ignoring");
            }
        }
        CharSequence charSequence = this.A;
        boolean z2 = menuItem instanceof cmn;
        if (z2) {
            ((cmn) menuItem).a(charSequence);
        } else {
            menuItem.setContentDescription(charSequence);
        }
        CharSequence charSequence2 = this.B;
        if (z2) {
            ((cmn) menuItem).b(charSequence2);
        } else {
        ...
        int i3 = this.o;
        if (z2) {
            ((cmn) menuItem).setAlphabeticShortcut(c, i3);
        } else {
        ...
        int i4 = this.q;
        if (z2) {
            ((cmn) menuItem).setNumericShortcut(c2, i4);
        } else {
        ...
        if (mode != null) {
            if (z2) {
                ((cmn) menuItem).setIconTintMode(mode);
            } else {
        ...
        if (colorStateList != null) {
            if (z2) {
                ((cmn) menuItem).setIconTintList(colorStateList);
            } else {
